...
describe('AdminService', () => {
  ...
  describe('ensureSuperAdminExists', () => {
    ...
    it('creates and saves super admin', async () => {
      ...
      (adminRepository.create as jest.Mock).mockReturnValue({ id: 1 });
      (adminRepository.save as jest.Mock).mockResolvedValue({});
    ...
    it('warns if duplicate email occurs', async () => {
      ...
      process.env.SUPER_ADMIN_FIRST_NAME = 'John';
      process.env.SUPER_ADMIN_LAST_NAME = 'Doe';
      (adminRepository.create as jest.Mock).mockReturnValue({});
      (adminRepository.save as jest.Mock).mockRejectedValue({ code: '23505' });
    ...
    it('throws unknown errors', async () => {
      ...
      process.env.SUPER_ADMIN_LAST_NAME = 'Doe';
      (adminRepository.create as jest.Mock).mockReturnValue({});
      (adminRepository.save as jest.Mock).mockRejectedValue(
        new Error('DB crash'),
      );
  ...
  describe('create', () => {
    it('creates a new admin', async () => {
      ...
      (adminRepository.create as jest.Mock).mockReturnValue(mockCreatedAdmin);
      (adminRepository.save as jest.Mock).mockResolvedValue(mockSavedAdmin);
  ...
  describe('findAll', () => {
    it('returns all admins', async () => {
      ...
      (adminRepository.find as jest.Mock).mockResolvedValue(fakeAdmins);
  ...
  describe('findOne', () => {
    it('returns admin if found', async () => {
      const admin = { id: '1' };
      (adminRepository.findOne as jest.Mock).mockResolvedValue(admin);
  ...
  describe('updateProfile', () => {
    it('updates with password', async () => {
      ...
      (adminRepository.preload as jest.Mock).mockResolvedValue(preloadedAdmin);
      (adminRepository.save as jest.Mock).mockResolvedValue(preloadedAdmin);
    ...
    it('throws if admin not found', async () => {
      (adminRepository.preload as jest.Mock).mockResolvedValue(null);
      const dto = {
        first_name: 'John',
        last_name: 'Doe',
        email: 'a@test.com',
  ...
  describe('updateRoleAndStatus', () => {
    ...
    it('sets approvedBy and approved_at if ACTIVE', async () => {
      ...
      (adminRepository.findOne as jest.Mock)
        .mockResolvedValueOnce(approver)
        .mockResolvedValueOnce(target);
      (adminRepository.save as jest.Mock).mockImplementation(
        (admin: Admin) => admin,
      );
      const dto: UpdateAdminRoleStatusDto = {
        approverId: '1',
  ...
  describe('remove', () => {
    it('removes admin if found', async () => {
      const admin = { id: '1' };
      (adminRepository.findOneBy as jest.Mock).mockResolvedValue(admin);
      (adminRepository.remove as jest.Mock).mockResolvedValue(admin);
      const result = await service.remove('1');
      expect(result).toEqual(admin);
    });

    it('throws if admin not found', async () => {
      (adminRepository.findOneBy as jest.Mock).mockResolvedValue(null);
